...
import {
  ...
  BRANDING_UPLOAD_URL_PREFIX,
} from "@/services/upload/branding-upload.service";

/**
 * GET /uploads/branding/:restaurantId/:filename
 *
 * Public read-only serving of locally uploaded restaurant logos. Storage is a
 * PRIVATE runtime directory (NOT Next's public/ folder); this route reads the
 * file from disk on every request so uploads are served immediately in dev,
 * `next start`, PM2 and standalone modes.
 *
 * Security:
 *  - both params are validated against strict safe patterns;
 *  - the resolved path is confined under BRANDING_UPLOAD_ROOT;
 *  - content-type is derived from the extension (png/jpg/webp only);
...
export async function GET(
  ...
  try {
    ...
    const match = /^([0-9a-fA-F-]{36})\.(png|jpg|webp)$/.exec(filename);
    if (!match) {
      return notFound();
    }
    ...
    const absolute = path.resolve(BRANDING_UPLOAD_ROOT, restaurantId, filename);
...
export const BRANDING_SERVE_PREFIX = BRANDING_UPLOAD_URL_PREFIX;